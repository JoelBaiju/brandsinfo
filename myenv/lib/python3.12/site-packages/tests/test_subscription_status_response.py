from dataclasses import asdict

import responses

from phonepe.sdk.pg.common.exceptions import PhonePeException, BadRequest
from phonepe.sdk.pg.common.http_client_modules.phonepe_response import PhonePeResponse
from phonepe.sdk.pg.env import Env, get_pg_base_url
from phonepe.sdk.pg.subscription.v2.models.request.auth_workflow_type import (
    AuthWorkflowType,
)
from phonepe.sdk.pg.subscription.v2.models.request.frequency import Frequency
from phonepe.sdk.pg.subscription.v2.models.response.subscription_status_response_v2 import (
    SubscriptionStatusResponseV2,
)
from phonepe.sdk.pg.subscription.v2.models.subscription_constants import (
    SUBSCRIPTION_STATUS_API,
)
from http import HTTPStatus

from tests.base_test_with_oauth import BaseTestWithOauth


class SubscriptionStatusResponseTest(BaseTestWithOauth):

    @responses.activate
    def test_subscription_status_success(self):
        merchant_subscription_id = "c61d7378-a081-44ab-9559-ad8563a24b49"
        response = (
            '{"merchantSubscriptionId": "123456", "state": "ACTIVE", "authWorkflowType": '
            + '"PENNY_DROP", "frequency": "ON_DEMAND"}'
        )
        url = SUBSCRIPTION_STATUS_API.format(
            merchant_subscription_id=merchant_subscription_id
        )
        responses.add(
            responses.GET,
            get_pg_base_url(Env.SANDBOX) + url,
            body=response,
            status=HTTPStatus.OK,
        )

        actual_response = BaseTestWithOauth.subscription_client.get_subscription_status(
            merchant_subscription_id
        )
        expected_response = SubscriptionStatusResponseV2(
            merchant_subscription_id="123456",
            state="ACTIVE",
            auth_workflow_type=AuthWorkflowType.PENNY_DROP,
            frequency=Frequency.ON_DEMAND,
        )
        assert actual_response == expected_response

    @responses.activate
    def test_subscription_status_failure(self):
        merchant_subscription_id = "c61d7378-a081-44ab-9559-ad8563a24b49"
        phonepe_response = PhonePeResponse(
            code="Bad Request", message="message", data={"a": "b"}
        )
        url = SUBSCRIPTION_STATUS_API.format(
            merchant_subscription_id=merchant_subscription_id
        )
        responses.add(
            responses.GET,
            get_pg_base_url(Env.SANDBOX) + url,
            json=asdict(phonepe_response),
            status=HTTPStatus.BAD_REQUEST,
        )

        with self.assertRaises(PhonePeException) as cm:
            BaseTestWithOauth.subscription_client.get_subscription_status(
                merchant_subscription_id
            )

        self.assertEqual(cm.exception.http_status_code, 400)
        self.assertEqual(cm.exception.code, "Bad Request")
        self.assertIsInstance(cm.exception, BadRequest)
