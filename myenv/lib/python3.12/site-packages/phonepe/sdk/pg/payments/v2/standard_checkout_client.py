...
from phonepe.sdk.common.models.response.callback_response import CallbackResponse
...
class StandardCheckoutClient(BaseClient):
    ...
    def get_instance(client_id: str, client_secret: str, client_version: int, env: Env = Env.SANDBOX,
        ...
        """
        should_publish_events = should_publish_events and env == Env.PRODUCTION
        if StandardCheckoutClient.instance is None:
            StandardCheckoutClient.instance = StandardCheckoutClient(client_id=client_id,
            ...
            StandardCheckoutClient.instance.event_publisher.send(init_event)
            return StandardCheckoutClient.instance
        ...
        cached_client_sha = calculate_hash(
            str(StandardCheckoutClient.instance.credential_config.client_id),
            str(StandardCheckoutClient.instance.credential_config.client_version),
            str(StandardCheckoutClient.instance.credential_config.client_secret),
            str(StandardCheckoutClient.instance.env),
            str(StandardCheckoutClient.instance.should_publish_events),
            str(FlowType.PG_CHECKOUT))

        if cached_client_sha == requested_client_sha:
            return StandardCheckoutClient.instance

        raise PhonePeException(
            message="Cannot re-initialize StandardCheckoutClient. Please utilize the existing Client object with "
