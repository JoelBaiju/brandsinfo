...
class TokenService:
    ...
    def _is_cached_token_valid(self):
        ...
        if TokenService.cached_token_data is None:
            return False

        issued_at = TokenService.cached_token_data.issued_at
        expires_at = TokenService.cached_token_data.expires_at
    ...
    def get_auth_token(self):
        if self._is_cached_token_valid():
            return (
                TokenService.cached_token_data.token_type
                + " "
                + TokenService.cached_token_data.access_token
            )
        try:
            token_data = self.fetch_token_from_phonepe().json()
            TokenService.cached_token_data = OauthResponse.from_dict(token_data)
            ...
            self.event_publisher.send(
                build_oauth_event_used_cached_token_failed(
                    cached_token_issued_at=TokenService.cached_token_data.issued_at,
                    cached_token_expires_at=TokenService.cached_token_data.expires_at,
